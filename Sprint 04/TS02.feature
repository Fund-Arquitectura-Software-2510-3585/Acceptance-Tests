Feature: Gestión de acceso con IAM en la API
  Como desarrollador,
  Quiero integrar IAM dentro de la API
  Para otorgar acceso adecuado a usuarios administradores y restringir funcionalidades según sus roles
  ...
  Scenario: Inicio de sesión de usuario no administrador
    Given que el usuario no tiene rol de administrador
  ...
  Scenario: Registro de usuario no administrador
    Given que el usuario no tiene permisos de administrador
